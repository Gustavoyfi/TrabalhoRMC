# Integrante - 1: Adryan Costa
# Integrante - 2: João Vitor
# Integrante - 3: Gustavo Yuri
...
def resolver_inequacao_2_grau(a, b, c, d):
    ...
    delta = b**2 - 4*a*(c - d)
    return -b + delta**(1/2)/2 and -b - delta**(1/2)/2
...
def funcao_tangente(x):
    #use a função trigonométrica disponível em Numpy
    tangente = funcao_seno(x) / funcao_cosseno(x)
    return tangente


def funcao_cotangente(x):
    #use a função trigonométrica que vc desenvolveu
    cotangente = (funcao_cosseno(x) / funcao_seno(x))
    return cotangente


def funcao_cossecante(x):
    #use a função trigonométrica que vc desenvolveu    
    cossecante = 1 / funcao_seno(x)
    return cossecante


def funcao_secante(x):
    #use a função trigonométrica que vc desenvolveu
    secante = 1 / funcao_cosseno(x)
    return secante
...
if __name__ == "__main__":
    ...
    while True: #repete o programa até 0 ser digitado
        ...
        if op==0: # encerrar o programa
            ...
            a, b, c = 2, 3, 5
            sol_ineq_1 = resolver_inequacao_1_grau(a, b, c)
            print(f"Solução da inequação {a}x + {b} > {c}: x > {sol_ineq_1}")
